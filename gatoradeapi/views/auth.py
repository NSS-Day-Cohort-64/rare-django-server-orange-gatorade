...
def register_user(request):
    ...
    if account_type is not None \
        ...
        if account_type == 'author':
            ...
            profile_img = request.data.get('profile_image_url', None)
            if bio is None:
                return Response(
                    {'message': 'You must provide a short bio for an author'},
                    status=status.HTTP_400_BAD_REQUEST
                )
            if profile_img is None or profile_img == '':
                profile_img = 'https://static.thenounproject.com/png/1768486-200.png'
